  ...
  return (
    <div className="w-full h-[70vh] relative overflow-hidden">
      ...
      >
        ...
        <ReactFastMarquee speed={isHovered ? 40 : 70} className="h-80 w-full">
          {reviewUserData.map((user) => (
            <div
              key={user.id}
              onMouseEnter={() => setIsHovered(true)}
              onMouseLeave={() => setIsHovered(false)}
              className="h-72 max-w-72 md:max-w-[700px]  rounded-3xl relative flex flex-col md:flex-row bg-white md:bg-zinc-950 mr-2 md:mr-5"
            >
              {/* Quote Icon */}
              ...
              <img
                src={user.userImg}
                alt={`user-${user.id}`}
                className="h-full w-full md:w-[35%] rounded-tl-3xl rounded-bl-3xl"
              />
